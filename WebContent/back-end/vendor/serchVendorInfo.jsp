                ...
                <div class="col-12">
                  ...
                  <div class="container">
                    ...
                    <c:if test="${not empty errorMsgs}">
                        ...
                        <h3>業者查詢</h3>
                        <div><a href="<%= request.getContextPath()%>/back-end/vendor/listAllVendor.jsp">顯示所有業者資料</a></div>
                        <br>
                        <div class="textline"> 
                            <form action="<%= request.getContextPath()%>/vendor/VendorServlet" method="post" accept-charset="UTF-8">編號查詢
                                <input type="text" name="vd_no">
                                <input type="hidden" name="action" value="getOnebyVd_no">
                                <input type="submit" value="送出" class="btn btn-outline-secondary btn-sm">
                            </form>
                        </div>
                        <div class="textline"> 
                            <form action="<%= request.getContextPath()%>/vendor/VendorServlet" method="post" accept-charset="UTF-8">信箱查詢
                                <input type="text" name="vd_email">
                                <input type="hidden" name="action" value="getOnebyVd_email">
                                <input type="submit" value="送出" class="btn btn-outline-secondary btn-sm">
                            </form>
                        </div>
                        <br>
                        <input type="button" value="返回業者查詢" onclick="location.href='<%= request.getContextPath()%>/back-end/vendor/serchVendorInfo.jsp'" class="btn btn-outline-secondary btn-sm">
                    </div>
                </div>
            </div>
          </div>
