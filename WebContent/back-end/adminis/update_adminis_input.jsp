...
<body id="page-top">
		                    ...
		                    <div class="col-lg-8">
		                        <div class="p-5">
		                            <div class="text-center">
		                                <h1 class="h4 text-gray-900 mb-4">管理員資料修改</h1>
		                            </div>
		                            ...
		                            <form class="user" ACTION="<%= request.getContextPath()%>/adminis/adminis.do" METHOD="post">
		                                ...
		                                <div class="form-group" style="color: black" >
		                                   &nbsp;&nbsp;&nbsp;&nbsp;<%=adminisVO.getAdminis_no()%>
		                                </div>                                
		                                
		                                <div class="form-group">
		                                    <input type="text" style="color: black" name="adminis_name"  class="form-control form-control-user"  placeholder="NAME"
		                               		 value="<%= (adminisVO == null)? "吳永志" : adminisVO.getAdminis_name()%>" />
		                                </div>
		                                
		                    			<div class="form-group">
		                    				<input type="password" style="color: black" name="adminis_pwd" class="form-control form-control-user" id="exampleInputPassword" placeholder="Password"
		            	      				value="<%= (adminisVO == null)? "123456" : adminisVO.getAdminis_pwd()%>" />
					    				</div>
		                                
		                                <div  class="form-group">
		                                    <input type="email" style="color: black"style="color: black" name="adminis_email" class="form-control form-control-user" id="exampleInputEmail" placeholder="Email Address"
		                            		value="<%= (adminisVO == null)? "EA103_99@gmail.com" : adminisVO.getAdminis_email()%>" />
		                                </div>
		                                
		                                <div class="form-group">
		                                    <input type="text" style="color: black" name="adminis_dept" class="form-control form-control-user"  placeholder="部門"
		                              		 value="<%= (adminisVO == null)?  "測試部" : adminisVO.getAdminis_dept()%>" />
		                                </div>
		 
		      							 <div class="form-group">
		                                    <input type="hidden" id="PV" name="adminis_pv"  class="form-control form-control-user"  
		                              		value="<%= (adminisVO == null)? "1234" : adminisVO.getAdminis_pv()%>" />
		                               	 </div> 
		                                ...
		                                <jsp:useBean id="authoritySvc"  scope="page" class="com.authority.model.AuthorityService" />
		                                 ...
		                                 <div class="form-group row p-4">
		                                   ...
		                                   <c:forEach  var="authorityVO" items="${authoritySvc.all}" >
		                                    ...
		                                    <input type="checkbox" name="test" class="custom-control-input" id="${authorityVO.auth_no}" value="<%= pvindex %>">
		                                    <label class="custom-control-label" for="${authorityVO.auth_no}">${authorityVO.auth_name}</label>
		                                  	</div>  
		                                  	<% pvindex*=2; %>                                 		
		                                   </c:forEach> 
		                                  </div>   
										...
										<input type="hidden" name="adminis_no" value="<%=adminisVO.getAdminis_no()%>"> 
										<input type="submit" value="送出修改" class="btn btn-primary btn-user btn-block">
		                          </form>
		                        </div>
		                    </div>
		                </div>
  ...
  <%@ include file="/back-end/logout.jsp" %> 
    ...
    var index = [];
	var pv =<%= adminisVO.getAdminis_pv()%>;
	var qqq;
	var listlength ='authorityVO'.length;
	var count1=1;
	var count2=0;
	var pvlist;
	...
	function substr() {
            ...
            pv= <%= adminisVO.getAdminis_pv()%>;
            str = pv.toString(2);
            str= padLeft(str,11);
            console.log(str);
            var i = 0;
            while (str.indexOf("1", i) !== -1) {
                var nub = str.indexOf("1", i)+1;
                index.push(nub.toString());
                i = nub;
            }
            return index;
        }
	...
	$(document).ready(function (){
		pvlist = substr();
     		<c:forEach  var="authorityVO" items="${authoritySvc.all}" >
			pvlist.forEach(function(item1){
					if(item1=='${authorityVO.auth_no}'){
						qqq = count1.toString() === '${authorityVO.auth_no}' ? listlength-count2+"" :'${authorityVO.auth_no}';
	...
	<script src="<%=request.getContextPath()%>/js/index-js.js"></script>
</body>
