				...
				<form method="post" action="<%=request.getContextPath()%>/equipment/equipment.do" name="form1" enctype="multipart/form-data" novalidate>
					<div class="table-responsive">
						<table class="table table-striped">
							<tbody>
								<tr class="form-group">
									<th scope="row" class="text-nowrap text-center align-middle">裝備編號：</th>
									<td><%=equipmentVO.getEqpt_no()%></td>
								</tr>
								<tr class="form-group">
									...
									<td>
										<select name="et_no" class="form-control" id="et_no">
											<c:forEach var="eqptTypeVO" items="${etSvc.all}">
												<option value="${eqptTypeVO.et_no}" ${(eqptTypeVO.et_no==equipmentVO.eqpt_etno)? 'selected':''}>${eqptTypeVO.et_name}
											</c:forEach>
										</select>
									</td>
								</tr>
								...
								<tr class="form-group">
									...
									<td>
										<select name="eqpt_stat" class="form-control" id="eqpt_stat">
											<option value="0" <%=(equipmentVO.getEqpt_stat() == 0) ? "selected" : ""%>>草稿</option>
											<option value="1" <%=(equipmentVO.getEqpt_stat() == 1) ? "selected" : ""%>>下架</option>
											<option value="2" <%=(equipmentVO.getEqpt_stat() == 2) ? "selected" : ""%>>上架</option>
										</select>
									</td>
								</tr>
								<tr class="form-group">
									...
									<td>
										<div class="form-row">
  											<div class="custom-file col-9">
    											...
    											<label class="custom-file-label" for="eqpt_pic">選擇檔案</label>
 									 		</div>
  											<div class="col">
						...
						<input type="hidden" name="eqpt_vdno" value="<%=equipmentVO.getEqpt_vdno()%>">
						<input type="submit" class="btn btn-outline-info" value="送出修改">
					</div>
				</form>
				
				<%	byte[] b = equipmentVO.getEqpt_pic(); // 由資料庫取得
	                if (b != null) {
