...
<body>
				...
				<form method="post" action="<%=request.getContextPath()%>/food/food.do" name="form1" enctype="multipart/form-data" novalidate>
					<div class="table-responsive">
						<table class="table table-striped">
							<tbody>
								...
								<tr class="form-group">
									...
									<td>
										...
										<c:forEach var="foodTypeVO" items="${ftSvc.all}">
											<option value="${foodTypeVO.ft_no}">${foodTypeVO.ft_name}
										</c:forEach>
										</select>
									</td>
								</tr>
								...
								<tr class="form-group">
									...
									<td>
										<textarea name="food_intro" class="form-control needs-validation" id="food_intro" maxlength="100" placeholder="上限100字" value="<%=(foodVO == null) ? "" : foodVO.getFood_intro()%>"></textarea>
								...
								<tr class="form-group">
									<th scope="row" class="text-nowrap text-center align-middle">食材照片：</th>
									<td>
										<div class="form-row">
  											<div class="custom-file col-9">
    											...
    											<label class="custom-file-label" for="food_pic" data-browse="瀏覽">選擇檔案</label>								
 									 		</div>
                                            <div class="col">
						...
						<div class="d-flex justify-content-between align-items-start">
							<input type="hidden" name="food_vdno" value="<%=vd_no%>">
	...
	<script>
        ...
        function init() {
            ...
            // 神奇小按鈕
            var food_nameArr = ['烤肉醬', '鮮魚', '豬排','生蠔', '螃蟹'];
            var food_priceArr = [50, 150, 200, 250, 300];
            var food_introArr = ['讚喔', '好吃', '真香', '鮮美', '肥美'];
            var food_name = document.getElementById("food_name");
            var food_price = document.getElementById("food_price");
            var food_intro = document.getElementById("food_intro");
            ...
            smBtn.addEventListener('click', function(e) {
            	...
            	var i = random()
            	food_name.value = food_nameArr[i];
            	food_price.value = food_priceArr[i];
            	food_intro.value = food_introArr[i];
            	var num = food_introArr[i].length;
            	var wordsNum = document.getElementById("wordsNum");
            	wordsNum.innerText = num;
            });
        }
