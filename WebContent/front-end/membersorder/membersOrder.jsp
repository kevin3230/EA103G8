...
<%
	...
	//session.setAttribute("mem_no", "M000000001");
	//測試用寫死
...
<%@ include file="/front-end/index/header.jsp" %>
		...
		<div class="row formargin">		
			<jsp:include page="//front-end/sidebar/memberSidebar.jsp" flush="true" />
			<div class="container col-9">
		        <div class="row justify-content-center my-2">
					...
					<div class="col-10 badge badge-secondary">
	                    <h3 class=" my-0">會員訂單瀏覽</h3>
	                </div>
	                <div class="col-10 px-0">
	                    <form action="#">
	                    <div class="form-row justify-content-center my-2">
	                        <div class="col-5 d-inline-block px-0">
	                            <label class="col-4 col-form-label col-form-label-sm px-0" for="startDate"><h5>開始日期</h5></label>
	                        ...
	                        <div class="col-5 d-inline-block px-0">
	                            <label class="col-4 col-form-label col-form-label-sm px-0" for="endDate"><h5>結束日期</h5></label>
    ...
    <script>
        ...
        function createTable(list){
        	var block = `
        		<td colspan="6">
        			<div class="col-11 my-2">
	                    ...
	                    <form action="<%=request.getContextPath() %>/carorder/EditOrderMasterServlet" id="myform" method="post">
		                    <input type="hidden" name="om_no" value="">
		                    <input type="hidden" name="action" value="editOrder">
	                    	<button class="badge badge-secondary edit" type="submit"><h6>修改</h6></button>
	                    </form>
        			</div>
        		</td>
        	`;
        	...
        	for (let i = 0; i < list.length; i++) {
                ...
                switch (list[i]["om_stat"]){
            	case "0" :
            		$("tbody").find($("[id='" + list[i]["om_no"] + "']")).find("#myform").remove();
            		break;
            	case "3" :
            		$("tbody").find($("[id='" + list[i]["om_no"] + "']")).find("#myform").remove();
            		break;
            	default:
            		//console.log("可以修改訂單");
            		$("tbody").find($("[id='" + list[i]["om_no"] + "']")).find("input").first().attr("value", list[i]["om_no"]);
            	}
        ...
        function reloadOneRow(list){
            ...
            switch (list["om_stat"]){
        	case "0" :
        		$("tbody").find($("[id='" + list["om_no"] + "']")).find("#myform").remove();
        		break;
        	case "3" :
        		$("tbody").find($("[id='" + list["om_no"] + "']")).find("#myform").remove();
        		break;
        	default:
        		//console.log("可以修改訂單");
        		$("tbody").find($("[id='" + list["om_no"] + "']")).find("input").first().attr("value", list["om_no"]);
        	}
