...
<script>
        ...
        function initMap() {
            ...
            var mapsiteList = [];
            $.ajax({
                url: "<%=request.getContextPath()%>/vendor/VendorSiteList",
                type: "GET",
                success: function(data){
                    mapsiteList = JSON.parse(data);
                    // console.log(mapsiteList);
                    showsite(mapsiteList);
                }
            });

            var markers = [];
            function showsite(mapsiteList){
            
                //建立marker物件&執行addMarker(e)
                function addMarker(e) {
                    markers[e] = new google.maps.Marker({
                        position: {
                            lat: parseFloat(mapsiteList[e].vd_lat),
                            lng: parseFloat(mapsiteList[e].vd_lon)
                        },
                        map: map,
                        icon: { //更改地圖標記樣式
                ...
                for (var i = 0; i < mapsiteList.length; i++) {
                    addMarker(i);

                    //建立infoWindow物件資訊視窗
                    var infowindow = new google.maps.InfoWindow({
                        position: {
                            lat: parseFloat(mapsiteList[i].vd_lat),
                            lng: parseFloat(mapsiteList[i].vd_lon)
                        },
                        maxWidth: 200
                    });
                    addInfoWindow(i);
                ...
                function addInfoWindow(j) { //額外寫一個函式，將變數i傳入，利用區域變數的特性，將變數i鎖在區域變數作用域上
                    ...
                    markers[j].addListener('click', function() {
                        ...
                        if (a > 0) {
                            //匯入infowindow內容
                            infowindow.setContent(`<a href="<%=request.getContextPath()%>/cgintro/cgintro.do?action=goToCGIntro&vd_no=`
                                                     + mapsiteList[j].vd_no + `">
                                                    <div class="cgname">` + mapsiteList[j].vd_cgname + `</div>
                                                    <div><span class="cgregion">電話: </span>` + mapsiteList[j].vd_cgtel + `</div>
                                                    <div><span class="cgregion">地址: </span>` + mapsiteList[j].vd_cgaddr + `</div>
                                                    <div><span class="cglatlon">經緯度: </span>(` + mapsiteList[j].vd_lat + `, ` + mapsiteList[j].vd_lon + `)</div>
                                                    </a>`);
                ...
                google.maps.event.addListener(map, 'idle', function() {
                    ...
                    $(".site").remove()
                    for (var j = 0; j < mapsiteList.length; j++) {
                        var latSite = mapsiteList[j].vd_lat,
                            lonSite = mapsiteList[j].vd_lon;            
                        if (boundsSWlat <= latSite && boundsNElat >= latSite &&
                            boundsSWlng <= lonSite && boundsNElng >= lonSite) {
                            $(".site_list").append(`
                                                    <a href="<%=request.getContextPath()%>/cgintro/cgintro.do?action=goToCGIntro&vd_no=`
                                                     + mapsiteList[j].vd_no + `">
                                                    <div class="site">
                                                    <div class="cgname">` + mapsiteList[j].vd_cgname + `</div>
                                                    <div><span class="cgregion">電話: </span>` + mapsiteList[j].vd_cgtel + `</div>
